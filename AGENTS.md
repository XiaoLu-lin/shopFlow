...
- `shopflow-admin-api`、`shopflow-wx-api`、`shopflow-core`、`shopflow-db`、`shopflow-admin`、`shopflow-h5-next`、`shopflow-wx` 默认都应有自己的 `AGENTS.md`。
...
- H5 前台目录为 `shopflow-h5-next`，推荐本地端口为 `6257`。
- H5 前台启动命令：`cd shopflow-h5-next && npm install && npm run dev:h5 -- --host 127.0.0.1 --port 6257`。
- H5 访问地址：`http://localhost:6257/#/`，用户中心页通常为 `http://localhost:6257/#/pages/user/index`。
- `shopflow-h5-next` 开发环境默认通过 `/wx` 代理到 `http://localhost:6914`，不再依赖旧 `shopflow-h5` 的 `vue.config.js` 代理配置。
