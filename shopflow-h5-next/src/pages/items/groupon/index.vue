  ...
  <view class="page">
    <view class="browse-shell">
        ...
        >
          ...
          <view class="browse-goods-body">
            ...
            <text class="browse-goods-brief">
              {{ resolveGoodsBrief(item.brief, '成团人数和优惠信息会轻量收在这里，不抢主商品信息。') }}
            </text>
            <view class="browse-info-tags">
              <text class="browse-info-tag">{{ item.grouponMember }} 人成团</text>
              <text class="browse-info-tag browse-info-tag--light">立减 {{ item.grouponDiscount }} 元</text>
            </view>
      ...
      <view v-else-if="!loading" class="browse-empty">
        ...
        <text v-if="emptyState.description" class="browse-empty-desc">{{ emptyState.description }}</text>
      </view>

      <view v-if="goodsList.length" class="browse-load-state">
        {{ loadingMore ? '正在继续加载...' : hasMore ? '上拉继续浏览更多拼团商品' : '已经到底了' }}
      </view>
    </view>
  </view>
</template>
...
const hero = resolveGoodsBrowseHero('groupon')
const emptyState = resolveGoodsBrowseEmptyState('groupon')
const showHeroDescription = shouldRenderBrowseHeroDescription('groupon')
...
async function bootstrap() {
  ...
  hasMore.value = false
  try {
    const result = await fetchGrouponList({ page: page.value, limit: 20 })
