        ...
        >
          ...
          <view class="browse-goods-body">
            ...
            <text class="browse-goods-brief">
              {{ resolveGoodsBrief(item.brief, '当前更受欢迎的商品，会按这组顺序集中展示。') }}
            </text>
...
import { fetchHotGoodsList, type GoodsListItem } from '@/entities/goods/api'
...
const hero = resolveGoodsBrowseHero('hot')
const emptyState = resolveGoodsBrowseEmptyState('hot')
const showHeroDescription = shouldRenderBrowseHeroDescription('hot')
...
async function bootstrap() {
  ...
  hasMore.value = false
  try {
    const result = await fetchHotGoodsList({ page: page.value, limit: 20 })
