...
function formatTime(dateTime){// 获取当前时间
  let date = dateTime+'00秒';
  let date1 = date.replace('年', '-');
  let date2 = date1.replace('月', '-');
  let date3 = date2.replace('日', ' ');
  let date4 = date3.replace('时', ':');
  let date5 = date4.replace('分', ':');
  let data6 = date5.replace('秒', '');
  return data6;
}
...
function getCurrentDate(){// 获取当前时间
  ...
  return currentYears+'年'+mm+'月'+dd+'日'+hh+'时'+min+'分';
}
