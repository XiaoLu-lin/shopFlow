...
public class GoodsCoreService {
    ...
    @Async
    void createGoodShareImage(LitemallGoods goods){
        //将生成的分享图片地址写入数据库
    ...
    public Object goodsUpdate(GoodsAllinone goodsAllinone) {
        ...
        if (!GoodsStatus.getIsOnSale(goods)){
            //删除赏金
            LitemallRewardTask rewardTask = commonService.findByRewardTaskGid(goods.getId());
            if (rewardTask != null) {
    ...
    public Object goodsDelete(String goodsId) {
        ...
        if (goods.getIsGroupon() != null && goods.getIsGroupon()){
            LitemallGrouponRules grouponRules = commonService.findByGrouponRulesGid(goodsId);
            //删除团购超时任务
            taskService.removeTask(new GrouponRuleExpiredTask(grouponRules));
            //设置团购立马过期
            GrouponRuleExpiredTask expiredTask = new GrouponRuleExpiredTask(grouponRules);
        ...
        //删除赏金
        LitemallRewardTask rewardTask = commonService.findByRewardTaskGid(goodsId);
        if (rewardTask != null) {
