...
import org.ysling.shopflow.core.system.enums.SystemMallConfig;
...
public class OrderCoreService {
     ...
     * @param order 订单
     */
    public void orderPaySuccess(ShopflowOrder order){
        //有赏金更新赏金
        ...
        // 支付成功，有团购信息，更新团购信息
        ShopflowGroupon groupon = commonService.findByGrouponOrderId(order.getId());
        if (groupon != null) {
            grouponCoreService.updateGrouponStatus(groupon);
        }else {
            if (order.getActualPrice().compareTo(BigDecimal.ZERO) <= 0 || !SystemConfig.isAutoWxPay()) {
                //跟新订单状态
     ...
     * 添加订单信息和订单商品信息
     */
    public void addOrderAndOrderGoods(ShopflowCart cartGoods, ShopflowOrder order, ShopflowUser user, Boolean isDeduction) {
        // 商品总价 = (商品数量 * 商品单价)
        ...
        // 订单商品
        ShopflowOrderGoods orderGoods = new ShopflowOrderGoods();
    ...
    public void reduceStock(String productId , Short number){
        ...
        QueryWrapper<ShopflowGoodsProduct> wrapper = new QueryWrapper<>();
        wrapper.eq(ShopflowGoodsProduct.ID , productId);
        wrapper.ge(ShopflowGoodsProduct.NUMBER , number);
        if (!goodsProductService.update(product , wrapper)){
     ...
     */
    public void reduceStock(ShopflowCart cartGoods){
        // 商品货品数量减少
     ...
     * @param order 订单
     */
    public void orderRelease(ShopflowOrder order) {
        // 商品货品数量增加
        List<ShopflowOrderGoods> orderGoodsList = commonService.queryByOrderGoodsOid(order.getId());
        for (ShopflowOrderGoods orderGoods : orderGoodsList) {
            ...
            //商品上架
            ShopflowGoods goods = goodsService.findById(orderGoods.getGoodsId());
        ...
        // 返还优惠券
        List<ShopflowCouponUser> couponUsers = commonService.queryByCouponUserOid(order.getId());
        for (ShopflowCouponUser couponUser: couponUsers) {
            // 优惠券状态设置为可使用
        ...
        //退款则发送退款订阅信息
        if (OrderStatus.isRefund(order) && order.getActualPrice().compareTo(BigDecimal.ZERO) > 0){
            //订单退款订阅通知
            ShopflowUser user = userService.findById(order.getUserId());
            subscribeMessageService.refundSubscribe(user.getOpenid(), order);
            //记录操作日志
            ActionLogHandler.logOrderSucceed("订单退款", "订单编号 " + order.getOrderSn());
        }
