...
public class RewardCoreService {
     ...
     * 修改赏金状态和订单状态
     */
    public void updateRewardStatus(ShopflowReward reward){
    ...
    public Object isReward(String rewardTaskId){
        ...
        ShopflowRewardTask rewardTask = rewardTaskService.findById(rewardTaskId);
        if (rewardTask == null) {
     ...
     * @param order 订单
     */
    public void addReward(String rewardLinkId, String userId , ShopflowOrder order){
        //添加赏金活动
        if (rewardLinkId != null && !"0".equals(rewardLinkId)){
            ShopflowReward reward = rewardService.findById(rewardLinkId);
            ShopflowRewardTask rewardTask = rewardTaskService.findById(reward.getTaskId());
            ShopflowReward newReward = new ShopflowReward();
