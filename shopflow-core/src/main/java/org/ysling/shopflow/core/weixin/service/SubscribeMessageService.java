...
public class SubscribeMessageService {
     ...
     */
    @Async
    public void shipSubscribe(String openId ,ShopflowOrder order){
     ...
     */
    @Async
    public void refundSubscribe(String openId ,ShopflowOrder order){
        // 模板数据 下面的数据，是需要根据申请的微信模板对应的，并不是每个人都一样
        List<WxMaSubscribeMessage.MsgData> data = new ArrayList<>();
     ...
     */
    @Async
    public void newOrderSubscribe(String openId ,ShopflowOrder order , ShopflowOrderGoods orderGoods){
        // 模板数据 下面的数据，是需要根据申请的微信模板对应的，并不是每个人都一样
        List<WxMaSubscribeMessage.MsgData> data = new ArrayList<>();
        data.add(new WxMaSubscribeMessage.MsgData("character_string1", order.getOrderSn()));
     ...
     */
    @Async
    public void newMessageSubscribe(String openId , ShopflowMessage message){
        // 模板数据 下面的数据，是需要根据申请的微信模板对应的，并不是每个人都一样
        List<WxMaSubscribeMessage.MsgData> data = new ArrayList<>();
