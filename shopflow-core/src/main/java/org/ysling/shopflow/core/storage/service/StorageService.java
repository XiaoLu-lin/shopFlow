package org.ysling.shopflow.core.storage.service;
/**
...
import org.ysling.shopflow.core.storage.storage.AbstractStorage;
...
public class StorageService {
     ...
     * @param fileName      文件名
     */
    public ShopflowStorage store(byte[] data, String fileName) {
        ByteArrayInputStream inputStream = new ByteArrayInputStream(data);
        String keyName = generateKey(inputStream, fileName);
        ShopflowStorage storage = isStorage(keyName, data.length);
        if (storage != null){
            return storage;
        }
     ...
     * @param file 文件对象
     */
    public ShopflowStorage store(File file){
        //添加到对象存储
        String keyName = generateKey(file, file.getName());
        ShopflowStorage storage = isStorage(keyName, file.length());
        if (storage != null){
            return storage;
        }
     ...
     * @param file 文件对象
     */
    public ShopflowStorage store(MultipartFile file) {
        try {
            String filename = Objects.requireNonNull(file.getOriginalFilename());
            String keyName = generateKey(file.getInputStream(), filename);
            ShopflowStorage storage = isStorage(keyName, file.getSize());
            if (storage != null){
                return storage;
            }
            abstractStorage.store(file, keyName);
     ...
     * @return 对象
     */
    public ShopflowStorage isStorage(String keyName, long size) {
     ...
     * 将对象存入数据库
     */
    private ShopflowStorage addStorage(String name, long size, String keyName){
        String url = generateUrl(keyName);
        this.picSecCheck(keyName, url);
        ShopflowStorage storage = getStorage(name, size, keyName);
     ...
     * 获取对象
     */
    private ShopflowStorage getStorage(String name, Long size, String keyName){
        String url = generateUrl(keyName);
        ShopflowStorage storageInfo = new ShopflowStorage();
