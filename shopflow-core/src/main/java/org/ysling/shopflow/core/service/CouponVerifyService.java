...
public class CouponVerifyService {
     ...
     * 检测优惠券是否适合
     */
    public ShopflowCoupon checkCoupon(String userId, String couponId, String userCouponId, List<ShopflowCart> cartList) {
        ...
        // 检测商品是否符合
        Map<String, List<ShopflowCart>> cartMap = new HashMap<>();
        //可使用优惠券的商品或分类
        ...
        if (goodType.equals(CouponGoodsType.GOODS_TYPE_CATEGORY.getStatus()) ||
            ...
            for (ShopflowCart cart : cartList) {
                ...
                List<ShopflowCart> carts = cartMap.get(key);
                if (carts == null) {
            ...
            for (String goodsId : goodsValueList) {
                List<ShopflowCart> carts = cartMap.get(goodsId);
                for (ShopflowCart cart : carts) {
