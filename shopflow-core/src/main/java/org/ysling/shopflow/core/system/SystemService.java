...
public class SystemService implements ApplicationRunner {
    ...
    public void initRedisConfig(){
        HashMap<String, ShopflowSystem> dataMap = new HashMap<>();
        List<ShopflowSystem> systemList = systemService.list();
        for (ShopflowSystem system :systemList) {
     ...
     */
    private void initDbSystem(List<ShopflowSystem> systemList){
        for (ShopflowSystem system : systemList) {
            saveOrUpdate(system);
        }
    }
     ...
     * @param system    配置信息
     */
    private void saveOrUpdate(ShopflowSystem system){
        QueryWrapper<ShopflowSystem> wrapper = new QueryWrapper<>();
        wrapper.eq(ShopflowSystem.NAME, system.getName());
        List<ShopflowSystem> systemList = systemService.list(wrapper);
        if (systemList.isEmpty()){
            if (!systemService.save(system)){
                throw new RuntimeException("系统配置添加失败：" + system.getName());
            }
        }else {
            for (ShopflowSystem item :systemList) {
                item.setValue(null);
            ...
            if (clazz.isEnum()){
                ...
                if (systemEnums != null){
                    ...
                    for (Object item :enumConstants) {
                        ...
                        ShopflowSystem system = JacksonUtil.parseObject(json, ShopflowSystem.class);
                        if (system != null){
    ...
    public Set<Class<?>> scanEntitiesWithAnnotation(Class<? extends Annotation> annotationClass) {
        ...
        for (BeanDefinition bd : scanner.findCandidateComponents("org.ysling.shopflow")) {
            try {
