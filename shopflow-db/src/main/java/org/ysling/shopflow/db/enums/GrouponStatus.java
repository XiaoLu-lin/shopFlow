...
public enum GrouponStatus implements Serializable {
     ...
     * 是否待开团
     */
    public static Boolean isNone(ShopflowGroupon groupon){
        return groupon.getStatus().equals(STATUS_NONE.getStatus());
    }
    /**
     * 团购中（已支付）
     */
    public static Boolean isOn(ShopflowGroupon groupon){
        return groupon.getStatus().equals(STATUS_ON.getStatus());
    }
    /**
     * 团购失败（待退款）
     */
    public static Boolean isFail(ShopflowGroupon groupon){
        return groupon.getStatus().equals(STATUS_FAIL.getStatus());
    }
    /**
     * 团购成功（待发货）
     */
    public static Boolean isSucceed(ShopflowGroupon groupon){
        return groupon.getStatus().equals(STATUS_SUCCEED.getStatus());
    }
    /**
     * 团购取消（未支付）
     */
    public static Boolean isCancel(ShopflowGroupon groupon){
