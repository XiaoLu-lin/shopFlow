package org.ysling.shopflow.db.enums;
/**
...
import org.ysling.shopflow.db.entity.OrderHandleOption;
...
public enum OrderStatus implements Serializable {
     ...
     * @return 返回描述
     */
    public static String orderStatusText(ShopflowOrder order) {
        if (order != null) {
    ...
    public static OrderHandleOption build(ShopflowOrder order) {
        Short status = order.getOrderStatus();
    ...
    public static Integer orderBasics(ShopflowOrder order) {
        Short status = order.getOrderStatus();
    ...
    /**判断是否已支付*/
    public static boolean hasPayed(ShopflowOrder order) {
        return  OrderStatus.isPayStatus(order) || OrderStatus.isBtlPayStatus(order);
    }

    /**判断是否未支付*/
    public static boolean isPayed(ShopflowOrder order) {
        return OrderStatus.isCreateStatus(order) || OrderStatus.isGrouponNoneStatus(order);
    }

    /**判断是否退款*/
    public static boolean isRefund(ShopflowOrder order) {
        return OrderStatus.isRefundConfirmStatus(order) || OrderStatus.isFinishAftersaleStatus(order);
    }

    /**判断是否可以删除 ， 如果订单不是关闭状态(已取消、系统取消、已退款、用户已确认、系统已确认)，则不能删除*/
    public static boolean hasDelete(ShopflowOrder order) {
    ...
    /**判断是否可以发货*/
    public static boolean hasShip(ShopflowOrder order) {
        return OrderStatus.isPayStatus(order) || OrderStatus.isBtlPayStatus(order)
                || OrderStatus.isGrouponSucceedStatus(order);
    }

    /**未付款*/
    public static boolean isCreateStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_CREATE.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**已取消*/
    public static boolean isCancelStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_USER_CANCEL.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**已取消（系统）*/
    public static boolean isAutoCancelStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_AUTO_CANCEL.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**已付款*/
    public static boolean isPayStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_PAY.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**线下付款*/
    public static boolean isBtlPayStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_BTL_PAY.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**订单取消，退款中*/
    public static boolean isRefundStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_REFUND.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**已退款*/
    public static boolean isRefundConfirmStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_REFUND_CONFIRM.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**待开团（未支付）*/
    public static boolean isGrouponNoneStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_GROUPON_NONE.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**团购中（已支付）*/
    public static boolean isGrouponOnStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_GROUPON_ON.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**团购失败（待退款）*/
    public static boolean isGrouponFailStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_GROUPON_FAIL.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**团购成功（待发货）*/
    public static boolean isGrouponSucceedStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_GROUPON_SUCCEED.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**已发货*/
    public static boolean isShipStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_SHIP.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**已收货*/
    public static boolean isConfirmStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_CONFIRM.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**已收货（系统）*/
    public static boolean isAutoConfirmStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_AUTO_CONFIRM.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**评论已超时*/
    public static boolean isCommentOvertimeStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_COMMENT_OVERTIME.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**交易完成*/
    public static boolean isOrderSucceedStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.ORDER_SUCCEED.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**售后申请中*/
    public static boolean isPutAftersaleStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_PUT_AFTERSALE.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**售后退款中*/
    public static boolean isDisposeAftersaleStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_DISPOSE_AFTERSALE.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**售后已退款*/
    public static boolean isFinishAftersaleStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_FINISH_AFTERSALE.getStatus().equals(shopflowOrder.getOrderStatus());
    }
    /**售后已拒绝*/
    public static boolean isRejectAftersaleStatus(ShopflowOrder shopflowOrder) {
        return OrderStatus.STATUS_REJECT_AFTERSALE.getStatus().equals(shopflowOrder.getOrderStatus());
    }
