...
import org.ysling.shopflow.db.service.impl.DynamicServiceImpl;
...
public class AdminDynamicService extends DynamicServiceImpl {
    ...
    @Cacheable(sync = true)
    public List<ShopflowDynamic> querySelective(DynamicListBody body) {
        QueryWrapper<ShopflowDynamic> wrapper = startPage(body);
        if (body.getUserId() != null) {
            wrapper.eq(ShopflowDynamic.USER_ID , body.getUserId());
        }
        if (StringUtils.hasText(body.getContent())){
            wrapper.like(ShopflowDynamic.CONTENT , body.getContent());
        }
