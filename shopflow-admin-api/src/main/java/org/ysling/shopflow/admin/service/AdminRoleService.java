...
import org.ysling.shopflow.db.service.impl.RoleServiceImpl;
...
public class AdminRoleService extends RoleServiceImpl {
     ...
     * @return 错误信息
     */
    public Object validate(ShopflowRole role) {
    ...
    @Cacheable(sync = true)
    public List<ShopflowRole> querySelective(RoleListBody body) {
        QueryWrapper<ShopflowRole> wrapper = startPage(body);
        if (StringUtils.hasText(body.getName())) {
            wrapper.like(ShopflowRole.NAME , body.getName());
        }
    ...
    public boolean checkExist(String name) {
        ...
        wrapper.eq(ShopflowRole.NAME , name);
        return exists(wrapper);
    }

    
    @Cacheable(sync = true)
    public List<ShopflowRole> queryAll() {
        return list();
    }
