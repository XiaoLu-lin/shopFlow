...
import org.ysling.shopflow.admin.annotation.context.PermissionsContext;
...
import org.ysling.shopflow.admin.model.role.body.RoleListBody;
import org.ysling.shopflow.admin.model.role.body.RoleUpdatePermissionsBody;
import org.ysling.shopflow.admin.model.role.result.PermissionsResult;
...
public class AdminRoleController {
    ...
    public Object options() {
        List<ShopflowRole> roleList = roleService.queryAll();
        List<BaseOption> options = new ArrayList<>(roleList.size());
        for (ShopflowRole role : roleList) {
    ...
    @PostMapping("/create")
    public Object create(@Valid @RequestBody ShopflowRole role) {
    ...
    @PostMapping("/update")
    public Object update(@Valid @RequestBody ShopflowRole role) {
        Object error = roleService.validate(role);
        if (error != null) {
            return error;
        }
    ...
    public Object getPermissions(@JsonBody String roleId) {
        ...
        ShopflowAdmin admin = adminService.findById(adminId);
    ...
    public Object updatePermissions(@Valid @RequestBody RoleUpdatePermissionsBody body) {
        ...
        // 整合权限
        HashSet<ShopflowPermission> permissionSet = new HashSet<>();
        for (String permission : body.getPermissions()) {
            ShopflowPermission shopflowPermission = new ShopflowPermission();
            shopflowPermission.setRoleId(body.getRoleId());
            shopflowPermission.setPermission(permission);
            permissionSet.add(shopflowPermission);
        }
