...
public class AdminTenantService extends TenantServiceImpl {
     ...
     * @return null
     */
    public Object validate(ShopflowTenant tenant) {
        if (tenant == null){
    ...
    @Cacheable(sync = true)
    public List<ShopflowTenant> querySelective(TenantListBody body) {
        QueryWrapper<ShopflowTenant> wrapper = startPage(body);
        if (body.getAddress() != null){
            wrapper.eq(ShopflowTenant.ADDRESS , body.getAddress());
        }
