...
public class AdminCouponService extends CouponServiceImpl {
    ...
    public Object validate(ShopflowCoupon coupon) {
    ...
    public ShopflowCoupon findByCode(String code) {
        ...
        wrapper.eq(ShopflowCoupon.TYPE , CouponType.TYPE_CODE.getStatus());
    ...
    @Cacheable(sync = true)
    public List<ShopflowCoupon> querySelective(CouponListBody body) {
        QueryWrapper<ShopflowCoupon> wrapper = startPage(body);
        if (body.getType() != null) {
            wrapper.eq(ShopflowCoupon.TYPE , body.getType());
        }
        if (body.getStatus() != null) {
            wrapper.eq(ShopflowCoupon.STATUS , body.getStatus());
        }
        if (StringUtils.hasText(body.getName())) {
            wrapper.like(ShopflowCoupon.NAME , body.getName());
        }
