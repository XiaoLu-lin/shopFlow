...
import org.ysling.litemall.admin.model.profile.body.ProfilePasswordBody;
import org.ysling.litemall.admin.model.profile.result.AdminUpdateBody;
import org.ysling.litemall.core.utils.bcrypt.CryptoUtil;
...
public class AdminProfileController {
    ...
    public Object password(@Valid @RequestBody ProfilePasswordBody body) {
        ...
        LitemallAdmin admin = adminService.findById(StpUtil.getLoginIdAsString());
        if (!CryptoUtil.matches(oldPassword, admin.getPassword())) {
            return ResponseUtil.fail(ResponseStatus.USER_ERROR_A0210);
        }

        admin.setPassword(CryptoUtil.encode(newPassword));
    ...
    public Object detail() {
        ...
        LitemallTenant tenant = tenantService.findById(admin.getTenantId());
