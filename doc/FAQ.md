...
这里主要是指shopflow-wx-api和shopflow-wx两个模块的相关问题。
...
2. 开发者在shopflow-wx和shopflow-core模块的appid信息设置成自己申请的信息。
...
微信开发者工具中修改appid以后，请关闭shopflow-wx项目或者微信开发者工具，重新启动导入shopflow-wx。
...
```
shopflow.wx.app-id=
shopflow.wx.app-secret=
shopflow.wx.mch-id=
shopflow.wx.mch-key=
shopflow.wx.notify-url=
```
...
这里主要是指shopflow-admin-api和shopflow-admin两个模块的相关问题。
...
2. 其次，需要明白报错的地方，是shopflow-admin/src/utils/request.js文件中;
...
shopflow-db模块的application-db.yaml资源文件中reasonable是true
...
这里主要是指shopflow-db、shopflow-core和shopflow-all模块三个模块的相关问题。
...
```
org.apache.ibatis.binding.BindingException: Invalid bound statement (not found): org.ysling.shopflow.db.mapper.XXXX
```
...
The error may exist in org/ysling/shopflow/db/mapper/SystemMapper.xml
The error may involve org.ysling.shopflow.db.mapper.SystemMapper.selectByExample-Inline
The error occurred while setting parameters
SQL: select id, key_name, key_value, add_time, update_time, deleted from shopflow_system
...
开发者直接（或使用Navicat）导入 shopflow.sql 时运行失败。
...
首先，请开发者确认已经手动创建好数据库和数据库用户，然后再导入 `shopflow-db/sql/shopflow.sql`。
...
因此，如果开发者直接导入 `shopflow.sql` 失败，建议先拆成两步处理：
* 先手动创建数据库、用户和权限；
* 再单独导入 `shopflow-db/sql/shopflow.sql`。

此外实际上，生产环境也更推荐由运维侧手动创建数据库、账号和权限，而不是直接依赖示例初始化流程。
...
应该是shopflow-admin模块的node_modules文件夹导致的。
node_modules是shopflow-admin模块所依赖的项目库，可能有近200M的文件。
而IDEA如果没有设置，则可能尝试对该文件夹进行解析索引，从而导致卡断。

解决方案：

1. 先关闭IDEA，然后删除shopflow-admin模块内的node_modules文件夹；
2. 然后分别创建空的node_modules文件夹；
3. 重新打开IDEA，并将shopflow-admin模块的node_modules文件夹设置为Excluded状态。
