...
* 小商城前端，即shopflow-wx模块
  * 微信小程序
* 小商城后端，即shopflow-wx-api模块
  * Spring Boot 2.x
  * Spring MVC
...
1. 在shopflow-core模块的src/main/resources的application-core.yml资源文件中设置
    ```
    shopflow
        wx
            app-id: 开发者申请的app-id
...
2. 在shopflow-wx模块的project.config.json文件中设置
...
4. 建议开发者关闭当前项目或者直接关闭微信开发者工具，重新打开（因为此时shopflow-wx模块的appid可能未更新）。
   编译运行，尝试微信登录
...
2. 在shopflow-core模块的src/main/resources的application-core.yml资源文件中设置
...
小商城后端服务每一次请求都会检测是否存在HTTP头部域`X-ShopFlow-User-Token`。
...
注意
> shopflow-wx模块代码基于nideshop-mini-program的commit版本[acbf6276eb27abc6a48887cddd223d7261f0088e](https://github.com/tumobi/nideshop-mini-program/commit/acbf6276eb27abc6a48887cddd223d7261f0088e)。
> 由于改动变化较大，因此之后shopflow-wx将独立开发，不会合并nideshop-mini-program的更新。
...
shopflow-wx模块采用storage来存储一些数据，以及支持组件间数据通信。
