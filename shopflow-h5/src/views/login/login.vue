<template>
	<div class="login">
		...
		<div class="store_name">ShopFlow H5</div>
	</div>
  ...
  methods: {
    ...
    login() {
      ...
      authLoginByAccount(loginData).then(res => {
        ...
        if (!window.localStorage.getItem('Authorization') && session.token) {
          setLocalStorage({
    ...
    getLoginData() {
      ...
      if (mobileReg.test(this.account)) {
        payload.mobile = this.account;
      } else if (emailReg.test(this.account)) {
        payload.email = this.account;
      } else {
        payload.username = this.account;
      }
      return payload;
    }
  }
