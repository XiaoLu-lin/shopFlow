...
import org.ysling.shopflow.wx.model.order.body.OrderListBody;
import org.ysling.shopflow.wx.model.order.result.UserOrderInfo;
...
public class WxOrderService extends OrderServiceImpl {
    ...
    @Cacheable(sync = true)
    public ShopflowOrder findById(String userId, String orderId) {
    ...
    @Cacheable(sync = true)
    public ShopflowOrder findByBrandId(String brandId, String orderId) {
        QueryWrapper<ShopflowOrder> wrapper = new QueryWrapper<>();
        wrapper.eq(ShopflowOrder.ID , orderId);
        wrapper.eq(ShopflowOrder.BRAND_ID , brandId);
        return getOne(wrapper);
    }
    ...
    public Boolean countByOutTradeNo(String userId, String outTradeNo) {
        QueryWrapper<ShopflowOrder> wrapper = new QueryWrapper<>();
        wrapper.eq(ShopflowOrder.USER_ID , userId);
        wrapper.eq(ShopflowOrder.OUT_TRADE_NO , outTradeNo);
        return exists(wrapper);
    }

    
    @Cacheable(sync = true)
    public List<ShopflowOrder> queryByOrderStatus(String userId, List<Short> orderStatus, OrderListBody body) {
    ...
    @Cacheable(sync = true)
    public List<ShopflowOrder> queryByBrandOrderStatus(String brandId, List<Short> orderStatus, BrandOrderListBody body) {
        QueryWrapper<ShopflowOrder> wrapper = startPage(body);
        wrapper.eq(ShopflowOrder.BRAND_ID , brandId);
        if (orderStatus != null && orderStatus.size() > 0){
            wrapper.in(ShopflowOrder.ORDER_STATUS , orderStatus);
    ...
    public UserOrderInfo orderInfo(String userId) {
        QueryWrapper<ShopflowOrder> wrapper = new QueryWrapper<>();
        wrapper.eq(ShopflowOrder.USER_ID , userId);
        List<ShopflowOrder> orders = queryAll(wrapper);
        Integer unpaid = 0;
        Integer unship = 0;
        Integer unrecv = 0;
        Integer uncomment = 0;
