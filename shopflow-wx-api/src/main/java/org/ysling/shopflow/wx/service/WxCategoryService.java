...
public class WxCategoryService extends CategoryServiceImpl {
    ...
    @Cacheable(sync = true)
    public List<ShopflowCategory> queryL2ByIds(List<String> ids) {
        QueryWrapper<ShopflowCategory> wrapper = new QueryWrapper<>();
        wrapper.in(ShopflowCategory.ID , ids);
        wrapper.eq(ShopflowCategory.LEVEL , "L2");
        wrapper.orderByDesc(ShopflowCategory.WEIGHT);
        return queryAll(wrapper);
    }
    ...
    @Cacheable(sync = true)
    public List<ShopflowCategory> queryChannel() {
        QueryWrapper<ShopflowCategory> wrapper = new QueryWrapper<>();
        wrapper.eq(ShopflowCategory.LEVEL , "L1");
        wrapper.orderByDesc(ShopflowCategory.WEIGHT);
        return queryAll(wrapper);
