...
import org.ysling.shopflow.db.enums.CommentType;
...
import org.ysling.shopflow.wx.model.comment.body.CommentSubmitBody;
import org.ysling.shopflow.wx.model.comment.result.CommentListResult;
...
@Slf4j
@Service
public class WxWebCommentService {
     ...
     * 评论列表
     */
    public Object list(String userId, CommentListBody body) {
        List<ShopflowComment> commentList = commentService.querySelective(body);
        List<CommentListResult> commentVoList = new ArrayList<>(commentList.size());
        for (ShopflowComment comment : commentList) {
     ...
     * 回复评论列表
     */
    public Object replyList(String userId, CommentReplyListBody body) {
        List<ShopflowComment> commentList = commentService.queryReplySelective(body);
        List<CommentListResult> commentVoList = new ArrayList<>(commentList.size());
        for (ShopflowComment comment : commentList) {
            CommentListResult result = new CommentListResult();
            result.setCommentId(comment.getId());
     ...
     * @return 评论数量
     */
    public Object count(Short commentType ,String valueId) {
     ...
     * @return 提交订单操作结果
     */
    public Object submit(String userId, CommentSubmitBody body) {
        ...
        //整合信息
        ShopflowComment comment = new ShopflowComment();
