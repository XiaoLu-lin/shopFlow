...
public class WxTopicService extends TopicServiceImpl {
    ...
    @Cacheable(sync = true)
    public List<ShopflowTopic> queryList(PageBody body) {
        return queryAll(startPage(body));
    }

    
    @Cacheable(sync = true)
    public List<ShopflowTopic> queryList(TopicListBody body) {
    ...
    @Cacheable(sync = true)
    public List<ShopflowTopic> queryRelatedList(String id, Integer limit) {
        QueryWrapper<ShopflowTopic> wrapper = startPage(new PageBody(limit));
        wrapper.notIn(ShopflowTopic.ID , id);
        wrapper.orderByDesc(ShopflowTopic.WEIGHT);
        return queryAll(wrapper);
    }
