package org.ysling.shopflow.wx.socket;
...
import org.ysling.shopflow.core.redis.util.Message;
...
public class WxWebSocketContext {
    ...
    public static void sendHintTo(Session session, String content) {
        ShopflowMessage message = new ShopflowMessage();
     ...
     */
    public static void broadcast(ShopflowMessage message, Session me) {
        // 创建消息
     ...
     * @param message 消息体
     */
    public static void sendMessage(Session session, ShopflowMessage message) {
        if (message == null){
     ...
     * @param message 消息体
     */
    public static void sendMessage(String userId, ShopflowMessage message) {
        // 获得用户对应的 Session
     ...
     */
    public static void sendMessage(String userId, ShopflowMessage message, boolean convertSend) {
        // 获得用户对应的 Session
        Session session = USER_SESSION_MAP.get(userId);
        if (session == null) {
            log.error("==> user({}) 不存在对应的 session", userId);
     ...
     * @return 消息
     */
    private static String buildTextMessage(ShopflowMessage message) {
     ...
     */
    public static void sendMessage(ShopflowMessage message ,Session session, String receiveUserId) {
        sendMessage(session , message);
     ...
     * @return 消息对象
     */
    public static ShopflowMessage getMessage(String body, String userId) {
        //心跳校验
        ...
        //解析json
        ShopflowMessage message;
        try {
            message = JSONUtil.toBean(body, ShopflowMessage.class);
