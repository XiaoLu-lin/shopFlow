...
public class WxGrouponService extends GrouponServiceImpl {
    ...
    public Object isGroupon(String grouponId){
        ShopflowGroupon groupon = findById(grouponId);
        if (groupon == null) {
    ...
    public Object isGroupon(String grouponRulesId , String grouponLinkId , String userId){
        //如果是团购项目,验证活动是否有效
        ShopflowGrouponRules rules = grouponRulesService.findById(grouponRulesId);
        //找不到记录
        if (rules == null) {
            return ResponseUtil.badArgument();
        }
        ...
        if (grouponLinkId != null && !grouponLinkId.equals("0")) {
            ...
            // （2）不允许参加自己开团的团购
            ShopflowGroupon groupon = queryById(userId, grouponLinkId);
            if(groupon!=null) {
    ...
    public void addGroupon(String orderId, String userId, String grouponRulesId , String grouponLinkId){
        if (grouponRulesId != null && !"0".equals(grouponRulesId)) {
            ShopflowGroupon groupon = new ShopflowGroupon();
            ...
            if (grouponLinkId != null && !"0".equals(grouponLinkId)) {
                //参与的团购记录
                ShopflowGroupon baseGroupon = findById(grouponLinkId);
    ...
    public List<ShopflowGroupon> queryJoinRecord(String grouponId) {
        ...
        wrapper.eq(ShopflowGroupon.STATUS , GrouponStatus.STATUS_ON.getStatus());
        wrapper.orderByDesc(ShopflowGroupon.ADD_TIME);
     ...
     */
    @Cacheable(sync = true)
    public ShopflowGroupon queryById(String userId, String id) {
        QueryWrapper<ShopflowGroupon> wrapper = new QueryWrapper<>();
        wrapper.eq(ShopflowGroupon.USER_ID , userId);
        wrapper.eq(ShopflowGroupon.ID , id);
        return getOne(wrapper , false);
    }

    @Cacheable(sync = true)
    public ShopflowGroupon findByOrderId(String orderId) {
        QueryWrapper<ShopflowGroupon> wrapper = new QueryWrapper<>();
    ...
    public Integer countGroupon(String grouponId) {
        QueryWrapper<ShopflowGroupon> wrapper = new QueryWrapper<>();
        wrapper.eq(ShopflowGroupon.GROUPON_ID , grouponId);
        wrapper.eq(ShopflowGroupon.STATUS , GrouponStatus.STATUS_NONE.getStatus());
