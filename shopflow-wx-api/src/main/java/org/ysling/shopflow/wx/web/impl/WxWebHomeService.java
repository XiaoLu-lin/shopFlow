...
import org.ysling.shopflow.core.system.enums.SystemWxConfig;
...
import org.ysling.shopflow.wx.model.home.body.HomeNavigateBody;
import org.ysling.shopflow.wx.model.home.result.HomeAboutResult;
import org.ysling.shopflow.wx.model.home.result.HomeIndexResult;
...
public class WxWebHomeService {
    ...
    public Object index(String userId, String appid) {
        ...
        // 广告列表
        Inheritable<List<ShopflowAd>> bannerTask = new Inheritable<>(
                () -> adService.queryIndex()
        );

        // 分类列表
        Inheritable<List<ShopflowCategory>> channelTask = new Inheritable<>(
                () -> categoryService.queryChannel()
        );

        // 优惠券列表
        Inheritable<List<ShopflowCoupon>> couponListTask = new Inheritable<>(
                () -> couponService.queryAvailableList(userId, SystemWxConfig.getCouponLimit())
        );

        // 新品首发
        Inheritable<List<ShopflowGoods>> newGoodsListTask = new Inheritable<>(
                () -> goodsService.queryByNew(SystemWxConfig.getNewLimit())
        );

        // 热卖专区
        Inheritable<List<ShopflowGoods>> hotGoodsListTask = new Inheritable<>(
                () -> goodsService.queryByHot(SystemWxConfig.getHotLimit())
        );

        // 赏金任务
        Inheritable<List<ShopflowRewardTask>> rewardTaskListTask = new Inheritable<>(
                () -> rewardTaskService.queryByReward(SystemWxConfig.getRewardLimit())
        );

        // 团购专区
        Inheritable<List<ShopflowGrouponRules>> grouponRuleListTask = new Inheritable<>(
                () -> grouponRulesService.queryByGroupon(SystemWxConfig.getGrouponLimit())
        );

        // 首页商品分页
        Inheritable<PageInfo<ShopflowGoods>> allGoodsListTask = new Inheritable<>(
    ...
    public Object isNavigate(HomeNavigateBody body) {
        ...
        if (Objects.equals(sceneType,"goodsId")){
            ShopflowGoods goods = goodsService.findById(sceneId);
            if (goods == null){
        ...
        if (Objects.equals(sceneType,"rewardId")){
            ShopflowReward reward = rewardService.findById(sceneId);
            if (reward == null) {
    ...
    public Object about() {
        ...
        ShopflowTenant tenant = tenantService.findById(tenantId);
        if (tenant != null){
